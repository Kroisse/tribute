...
있는 표준 타입과 함수는 장기적으로 file-based module system을 사용하고, ambient 또는
intrinsic 성질처럼 compiler가 보장해야 하는 metadata만 builtin registry에 둔다.

현재 embedded prelude의 `List(a)`는 source로 선언되지만 list literal syntax는 그
선언의 stable semantic identity를 compiler metadata로 보존한다. 따라서 사용자 모듈의
동명 `List`가 일반 name resolution에서 prelude binding을 shadow하더라도 `[...]`는
사용자 타입에 capture되지 않는다. 이 규칙은 일반 이름 해소의 shadowing 규칙을
바꾸지 않고 compiler-owned syntax의 canonical target만 고정한다.
...
annotation을 포함한 모든 name-resolution 경로에서 동일하게 적용한다.
