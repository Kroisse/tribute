...
// Source-visible operations are specified in terms of sequence views.
pub mod List {
    pub fn prepend(value: a, tail: List(a)) -> List(a) { ... }
    pub fn is_empty(xs: List(a)) -> Bool { ... }
    pub fn len(xs: List(a)) -> Nat { ... }
    pub fn map(xs: List(a), f: fn(a) -> b) -> List(b) { ... }
    pub fn filter(xs: List(a), p: fn(a) -> Bool) -> List(a) { ... }
}
```

M1 source API는 dynamic construction을 위해
`List::prepend(value, tail) -> List(a)`만 추가로 보장한다. Prelude는 이 public
function을 compiler-provided intrinsic implementation으로 선언하며 shared
