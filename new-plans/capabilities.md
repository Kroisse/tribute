...
| Opaque `List` literals, `List::prepend`, and `[]`/exact/prefix-rest patterns | **compile-only** | **native-run** | **unsupported** | `test_list_literals_use_shared_sequence_ops` and `test_list_sequence_patterns_use_shared_observation_ops` assert representation-independent `list.*` IR in [`expr_coverage.rs`](../crates/tribute-front/tests/expr_coverage.rs). The `test_native_list_*` cases in [`e2e_native.rs`](../tests/e2e_native.rs) execute left-to-right exactly-once literals, recursive dynamic `List(String)` construction through public `List::prepend`, empty/exact/rest observation, persistent tails, and reference-element ownership. No Wasm `list.*` lowering exists, so this row makes no Wasm compilation or execution claim. |
