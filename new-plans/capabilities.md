...
| Byte-wise `String` equality (`==` and `!=`) | **compile-only** | **native-run** | **not-yet-verified** | `test_string_equality_operators` verifies frontend resolution and lowering in [`expr_coverage.rs`](../crates/tribute-front/tests/expr_coverage.rs). `test_native_string_equality` executes equal, unequal, empty, Unicode, shared, and differently shaped ropes and both operators in [`e2e_native.rs`](../tests/e2e_native.rs). No focused Wasm execution evidence was added. |
