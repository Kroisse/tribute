...
- **HIR → Cranelift IR pipeline**: Basic CodeGenerator and FunctionLowerer
...
1. **TrString 3-mode system**: Need enum-based inline/static/heap modes
2. **String interpolation**: Complex interpolation compilation missing
3. **Pattern matching**: Match expressions not implemented
4. **Main function**: Top-level expression evaluation incomplete
5. **trbc integration**: Compiler binary not connected
6. **Linking pipeline**: Cannot generate executable binaries
...
#[repr(C)]
pub enum TrString {
    // Mode 1: Inline - strings ≤ 23 bytes stored directly
    Inline { 
        data: [u8; 23], 
        len: u8 
    },
...
### Benefits
- **Memory efficiency**: 23-byte strings need no heap allocation
...
1. **Enum Structure Design**
   ```rust
   // Ensure C ABI compatibility and 24-byte size
   #[repr(C)]
   pub enum TrString {
       Inline { data: [u8; 23], len: u8 },
       Static { offset: u32, len: usize },
       Heap { data_index: u32, len: usize },
   }
   ```

2. **Runtime Functions Update**
   - `tr_string_new_inline(data: *const u8, len: u8) -> TrString`
   - `tr_string_new_static(offset: u32, len: usize) -> TrString`
   - `tr_string_new_heap(data: *const u8, len: usize) -> TrString`
   - `tr_string_as_str(string: *const TrString) -> (*const u8, usize)`

3. **Automatic Mode Selection**
   ```rust
   fn create_string(text: &str, is_compile_time: bool) -> TrString {
       match (text.len(), is_compile_time) {
           (0..=23, _) => TrString::new_inline(text),
           (_, true) => TrString::new_static(text),
           (_, false) => TrString::new_heap(text),
       }
   }
   ```
...
3. **Compiler String Literal Processing**
   - Short literals → Inline mode generation
   - Long literals → Static mode with offset
   - Runtime strings → Heap mode calls
...
```
Total size: 24 bytes (same as current)
Alignment: 8 bytes

Inline:  [discriminant:8][data:23][len:1]
Static:  [discriminant:8][offset:4][len:8][padding:4]
Heap:    [discriminant:8][index:4][len:8][padding:4]
```
...
fn select_string_mode(text: &str, context: StringContext) -> StringMode {
    match context {
        StringContext::Literal if text.len() <= 23 => StringMode::Inline,
...
### Memory Efficiency
- **Inline strings**: 100% heap allocation savings (for strings ≤ 23 bytes)
