...
use melior::{dialect::DialectRegistry, ir::Location, utility::register_all_dialects, Context};
use std::path::Path;
...
impl TributeCodegen {
    ...
    pub fn new() -> Result<Self> {
        ...
        mlir_context.append_dialect_registry(&registry);
    ...
    pub fn compile_hir(
        ...
        let mlir_module =
            hir_to_mlir::mlir_module_to_melior(db, mlir_module_data, &self.mlir_context, location)?;
...
mod tests {
    ...
    fn test_compile_arithmetic_function() {
        ...
        assert!(
            result.is_ok(),
            "Arithmetic compilation should succeed: {:?}",
            result
        );
        ...
        assert!(
            output_path.exists(),
            "Arithmetic output file should be created"
        );
    }
    ...
    fn test_debug_all_operations() {
        ...
        assert!(
            result.is_ok(),
            "All operations compilation should succeed: {:?}",
            result
        );
    }
    ...
    fn test_mlir_module_generation() {
        ...
        let hir_program =
            lower_source_to_hir(&db, source_file).expect("Should parse and lower to HIR");
        ...
        let has_addition = body_ops
            .iter()
            .any(|op| matches!(op, MlirOperation::Call { func, .. } if func == "builtin_+"));
        assert!(has_addition, "Should contain addition operation");

        // Check for return operation
        let has_return = body_ops
            .iter()
            .any(|op| matches!(op, MlirOperation::Return { .. }));
    ...
    fn test_mlir_number_boxing() {
        ...
        let has_number_boxing = body_ops
            .iter()
            .any(|op| matches!(op, MlirOperation::BoxNumber { value: 42 }));
    ...
    fn test_end_to_end_runtime_integration() {
        ...
        use tribute_runtime::{
            tribute_add_boxed, tribute_box_number, tribute_release, tribute_unbox_number,
        };
        ...
        assert!(
            result.is_ok(),
            "Integration test compilation should succeed: {:?}",
            result
        );
        assert!(
            output_path.exists(),
            "Integration test output should be created"
        );
    }
    ...
    fn test_compilation_empty_input() {
        ...
        assert!(
            result.is_ok(),
            "Empty source should compile successfully: {:?}",
            result
        );
        ...
        assert!(
            output_path.exists(),
            "Empty compilation should still create output file"
        );
    }
    ...
    fn test_compile_complex_program() {
        ...
        assert!(
            output_path.exists(),
            "Complex program output should be created"
        );
    }

    #[test]
    fn test_debug_list_operations() {
        use super::hir_to_mlir::{MlirListOperation, MlirOperation};
        ...
        for (i, op) in operations.iter().enumerate() {
            match op {
                MlirOperation::ListOp { operation } => match operation {
