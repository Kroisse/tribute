    ...
    for (type_idx, func_ty) in &module_info.call_indirect_types {
        ...
        debug!(
            ...
            params_refs
                .iter()
                .map(|t| {
                    let d = ctx.get_type(*t);
                    format!("{}.{}", d.dialect, d.name)
                })
                .collect::<Vec<_>>(),
            signature_results
                .iter()
                .map(|ty| format!("{}.{}", ctx.get_type(*ty).dialect, ctx.get_type(*ty).name))
                .collect::<Vec<_>>()
        );
        let params = params_refs
            .iter()
...
fn assign_locals_in_region(
    ...
    for &block_ref in &ctx.region(region).blocks {
        ...
        for &block_arg in block_args.iter() {
            ...
            let ty_data = ctx.get_type(arg_ty);
            tracing::debug!(
...
fn emit_op_nested(
    ...
    if wasm_dialect::Nop::matches(ctx, op) {
        if let Some(&result_ty) = ctx.op_result_types(op).first() {
            let ty_data = ctx.get_type(result_ty);
...
mod tests {
    ...
    const WASMTIME_CLI_REQUIRED: &str = "Wasmtime CLI is required; install the prebuilt 45.0.1 release with `cargo binstall --strategies crate-meta-data,quick-install wasmtime-cli@45.0.1`";
  ...
  wasm.func @caller(%table_index: core.i32, %value: wasm.structref) -> core.i32 {
    ...
    wasm.return %result
        ...
        assert_eq!(ctx.get_type(params[0]).name, Symbol::new("anyref"));
        assert_eq!(result.len(), 1);
        assert_eq!(ctx.get_type(result[0]).name, Symbol::new("i32"));
