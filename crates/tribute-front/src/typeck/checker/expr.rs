...
impl<'db> TypeChecker<'db> {
    ...
    pub(crate) fn check_expr_with_ctx(
        ...
        let ty = match &*expr.kind {
            ...
            ExprKind::Handle { body, handlers } => {
                ...
                ctx.merge_effect_at(result_effect, expr.id);
    ...
    fn generalize_and_bind_pattern_with_ctx(
        ...
        } in bindings
        {
            let scheme = if should_generalize {
                ...
                let (union_types, union_rows) = solver.row_union_variables(&retained);
    ...
    pub(super) fn check_exhaustiveness(
        ...
        if last_arm.guard.is_none() && self.is_catch_all_pattern(&last_arm.pattern) {
            return true; // Exhaustive via catch-all
        }
