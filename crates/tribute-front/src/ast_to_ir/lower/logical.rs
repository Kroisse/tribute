...
/// Seed worker conventions from the concrete effects of each source body.
/// Semantic function types stay untouched: a pure worker can still expose an
/// open-row callable type at first-class call sites.
...
fn lower_function<'db>(
    ...
    let root_export_convention = crate::is_root_main(function.name, ctx.module_path().len() == 1)
        .then(|| {
            FuncSignature::lookup_logical(ctx, ir, ctx.qualify_name(function.name))
