...
fn handler_pattern_to_region<'db>(ctx: &CstLoweringCtx<'db>, node: Node) -> Region<'db> {
    ...
    if let Some(op_path) = operation_path {
        ...
        // Create handler_suspend with continuation_type attribute for type inference
        // The type variable will be constrained by typeck to the actual continuation type
        let continuation_type_var = next_continuation_type_var(ctx.db);
        handler_suspend_region_with_type(
            ctx.db,
            location,
            ability_ref,
            op_name,
...
// =============================================================================
// Handler Pattern Type Variable Generation
// =============================================================================

use std::sync::atomic::{AtomicU64, Ordering};

/// Counter for generating unique continuation type variable IDs.
/// Start from a high number to avoid collision with type variables generated by typeck.
/// The typeck's `seed_var_counters` will find these and set its counter accordingly.
static CONTINUATION_TYPE_VAR_COUNTER: AtomicU64 = AtomicU64::new(1_000_000);

/// Generate a fresh type variable for continuation type.
fn next_continuation_type_var<'db>(db: &'db dyn salsa::Database) -> Type<'db> {
    let id = CONTINUATION_TYPE_VAR_COUNTER.fetch_add(1, Ordering::SeqCst);
    tribute::type_var_with_id(db, id)
}

/// Create a handler_suspend region with a continuation_type attribute.
///
/// This is similar to `tribute_pat::helpers::handler_suspend_region`, but additionally
/// stores the continuation type variable as an attribute for type inference.
fn handler_suspend_region_with_type<'db>(
    db: &'db dyn salsa::Database,
    location: trunk_ir::Location<'db>,
    ability_ref: Type<'db>,
    op_name: Symbol,
    ...
    // Create the base handler_suspend operation
    let base_op = tribute_pat::handler_suspend(
        db,
        location,
        ability_ref,
        op_name,
    ...
    // Add continuation_type attribute
    let op_with_type = base_op
        .as_operation()
        .modify(db)
        .attr(CONTINUATION_TYPE(), Attribute::Type(continuation_type))
        .build();

    // Wrap in a region
    tribute_pat::helpers::single_op_region(db, location, op_with_type)
}
