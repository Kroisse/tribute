...
/// - Field 0: resume function (anyref) - function to call when resuming (stored as anyref for uniformity)
...
///
/// Note: Field 0 uses anyref instead of funcref to match the types generated by
/// trampoline_to_adt pass. The resume function is cast to funcref when called.
///
...
pub fn register_continuation_type<'db>(
    ...
    // Create continuation struct type: (anyref, anyref, i32, anyref)
    // All ref fields use anyref for uniformity with trampoline_to_adt output
    let def = GcTypeDef::Struct(vec![
        FieldType {
            element_type: StorageType::Val(ValType::Ref(RefType::ANYREF)),
            mutable: false,
    ...
    fn test_register_continuation_type() {
        ...
        match &types[0] {
            GcTypeDef::Struct(fields) => {
                ...
                // Field 0: anyref (resume function, stored as anyref for uniformity)
                assert!(matches!(
                    fields[0].element_type,
                    StorageType::Val(ValType::Ref(RefType::ANYREF))
                ));
