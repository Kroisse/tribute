...
/// Pattern for `cont.handler_dispatch` -> yield-checking if/else
///
/// This operation is generated by case_lowering for handler patterns.
/// It dispatches based on the yield state:
/// - If yield_state == 0 (not yielding): execute done_body
/// - If yield_state != 0 (yielding): execute suspend_body
///
/// The suspend_body can access:
/// - $yield_cont: the captured continuation
/// - $yield_value: the value passed with the effect
...
impl RewritePattern for HandlerDispatchPattern {
    fn match_and_rewrite<'db>(
        ...
        // Get the done_body and suspend_body regions
        let done_body = dispatch.done_body(db);
        let suspend_body = dispatch.suspend_body(db);
        ...
        // - then: suspend_body (yielding case)
        // - else: done_body (normal completion case)
        //
        // Note: wasm.if condition is "if true" so:
        // - if (yield_state != 0) -> then branch = suspend_body
        // - else -> done_body
        let if_op = wasm::r#if(
            db,
            location,
