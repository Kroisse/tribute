...
/// Arena pattern: Lower `cont.push_prompt` → body outlining + `__tribute_prompt` call.
pub(crate) struct ArenaLowerPushPromptPattern {
    body_counter: Cell<u32>,
}

impl ArenaLowerPushPromptPattern {
    pub(crate) fn new() -> Self {
        Self {
            body_counter: Cell::new(0),
        }
...
impl ArenaRewritePattern for ArenaLowerPushPromptPattern {
    fn match_and_rewrite(
        &self,
        ctx: &mut IrContext,
        op: OpRef,
        ...
        let live_ins = compute_live_ins_arena(ctx, body);
        ...
        // Generate outlined body function and add to module
        let outlined_func = generate_outlined_body_arena(ctx, &body_name, &live_ins, body, loc);
        rewriter.add_module_op(outlined_func);
        ...
        let env_val = if live_ins.is_empty() {
            ...
            let env_struct_ty = build_env_struct_type_arena(ctx, &field_types);
            let struct_new =
    ...
    fn name(&self) -> &'static str {
        "ArenaLowerPushPromptPattern"
    }
}

/// Compute live-in values for a region (arena version).
fn compute_live_ins_arena(ctx: &IrContext, region: RegionRef) -> Vec<(ValueRef, TypeRef)> {
    ...
    collect_defined_in_region_arena(ctx, region, &mut defined);

    // Collect all used values
    let blocks = ctx.region(region).blocks.clone();
    for &block in &blocks {
        for &op in &ctx.block(block).ops.clone() {
            collect_used_values_arena(ctx, op, &mut used, &mut seen_used);
        }
    }
...
fn collect_defined_in_region_arena(
    ctx: &IrContext,
    region: RegionRef,
    ...
    for &block in &blocks {
        ...
        for &op in &ops {
            ...
            for r in regions {
                collect_defined_in_region_arena(ctx, r, defined);
            }
        }
    }
}

fn collect_used_values_arena(
    ctx: &IrContext,
    op: OpRef,
    ...
    for r in regions {
        ...
        for &block in &blocks {
            ...
            for &nested_op in &ops {
                collect_used_values_arena(ctx, nested_op, used, seen);
            }
        }
    }
}

/// Generate a `func.func` for an outlined prompt body (arena version).
///
/// Uses explicit value remapping instead of `replace_all_uses` to avoid
/// corrupting values used outside the body region.
fn generate_outlined_body_arena(
    ctx: &mut IrContext,
    name: &str,
...
/// Build an env struct type with the given field types (arena version).
///
...
/// - params: field types
fn build_env_struct_type_arena(ctx: &mut IrContext, field_types: &[TypeRef]) -> TypeRef {
