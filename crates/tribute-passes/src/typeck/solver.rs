...
impl<'db> TypeSolver<'db> {
    ...
    pub fn unify_types(&mut self, t1: Type<'db>, t2: Type<'db>) -> SolveResult<'db, ()> {
        ...
        match (t1_is_var, t2_is_var) {
            (true, _) => {
                // Bind t1 to t2
                if let Some(Attribute::IntBits(id)) = t1.get_attr(self.db, "id") {
                    // Occurs check
            ...
            (false, true) => {
                // Bind t2 to t1
                if let Some(Attribute::IntBits(id)) = t2.get_attr(self.db, "id") {
                    // Occurs check
    ...
    fn occurs_in(&self, var_id: u64, ty: Type<'db>) -> bool {
        ...
        if ty::is_var(self.db, ty)
            && let Some(Attribute::IntBits(id)) = ty.get_attr(self.db, "id")
        {
            return *id == var_id;
        }
