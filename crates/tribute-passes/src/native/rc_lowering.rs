...
/// Arena: Name of the deep release dispatch function.
const ARENA_DEEP_RELEASE_FN: &str = "__tribute_deep_release";

/// Arena: Lower all `tribute_rt.retain` and `tribute_rt.release` operations to
/// inline `clif.*` operations.
pub fn lower_rc_arena(ctx: &mut IrContext, module: ArenaModule) {
    ...
    for op in module_ops {
        if let Ok(_func_op) = arena_clif::Func::from_op(ctx, op) {
            let regions: Vec<RegionRef> = ctx.op(op).regions.to_vec();
            for region in regions {
                lower_rc_in_region_arena(ctx, region);
            }
        }
    }
}
...
fn lower_rc_in_region_arena(ctx: &mut IrContext, region: RegionRef) {
    ...
    for block in blocks {
        ...
        lower_rc_in_block_arena(ctx, region, block);
        ...
        for op in ops {
            ...
            for nested_region in nested {
                lower_rc_in_region_arena(ctx, nested_region);
            }
        }
    }
...
/// Arena: Lower RC ops in a single block, potentially splitting it.
///
...
fn lower_rc_in_block_arena(ctx: &mut IrContext, region: RegionRef, block: BlockRef) {
    let ops: Vec<OpRef> = ctx.block(block).ops.to_vec();
    let loc = ctx.block(block).location;

    // Quick check: any RC ops?
    let has_rc_ops = ops.iter().any(|&op| {
        arena_tribute_rt::Retain::from_op(ctx, op).is_ok()
            || arena_tribute_rt::Release::from_op(ctx, op).is_ok()
    });
    ...
    while i < ops.len() {
        ...
        if let Ok(_retain_op) = arena_tribute_rt::Retain::from_op(ctx, op) {
            ...
            let null_val = arena_clif::iconst(ctx, loc, ptr_ty, 0);
            ...
            let brif_op =
                arena_clif::brif(ctx, loc, is_null.result(ctx), skip_block, do_retain_block);
            ctx.push_op(current_block, brif_op.op_ref());

            // Generate retain RC ops in do_retain_block
            gen_retain_rc_ops_arena(ctx, loc, do_retain_block, ptr, ptr_ty, i64_ty, i32_ty);

            // Jump from do_retain to skip
            let jump = arena_clif::jump(ctx, loc, [], skip_block);
            ...
            return;
        } else if let Ok(release_op) = arena_tribute_rt::Release::from_op(ctx, op) {
            let ptr = ctx.op_operands(op).to_vec()[0];
            ...
            let brif_op =
                arena_clif::brif(ctx, loc, is_null.result(ctx), skip_block, do_release_block);
            ctx.push_op(current_block, brif_op.op_ref());

            // Generate release decrement ops in do_release_block
            let is_zero_val = gen_release_decrement_arena(
                ctx,
                loc,
                do_release_block,
                ptr,
            ...
            let zero_brif = arena_clif::brif(ctx, loc, is_zero_val, free_block, skip_block);
            ctx.push_op(do_release_block, zero_brif.op_ref());

            // Generate free block ops
            gen_deep_release_call_arena(
...
/// Arena: Generate retain RC ops (load, increment, store) in a block.
fn gen_retain_rc_ops_arena(
    ctx: &mut IrContext,
    ...
    let rc_addr = arena_clif::isub(ctx, loc, ptr, hdr_sz.result(ctx), ptr_ty);
    ctx.push_op(block, rc_addr.op_ref());

    // rc = load(rc_addr, offset=0)
    let rc = arena_clif::load(ctx, loc, rc_addr.result(ctx), i32_ty, 0);
    ctx.push_op(block, rc.op_ref());

    // new_rc = rc + 1
    let one = arena_clif::iconst(ctx, loc, i32_ty, 1);
    ctx.push_op(block, one.op_ref());
    let new_rc = arena_clif::iadd(ctx, loc, rc.result(ctx), one.result(ctx), i32_ty);
    ctx.push_op(block, new_rc.op_ref());

    // store(new_rc, rc_addr, offset=0)
    let store = arena_clif::store(ctx, loc, new_rc.result(ctx), rc_addr.result(ctx), 0);
...
fn gen_release_decrement_arena(
    ...
    let one = arena_clif::iconst(ctx, loc, i32_ty, 1);
    ctx.push_op(block, one.op_ref());
    let new_rc = arena_clif::isub(ctx, loc, rc.result(ctx), one.result(ctx), i32_ty);
    ctx.push_op(block, new_rc.op_ref());
    ...
    // is_zero = icmp(new_rc, 0, eq)
    let zero = arena_clif::iconst(ctx, loc, i32_ty, 0);
    ctx.push_op(block, zero.op_ref());
    let is_zero = arena_clif::icmp(
        ctx,
        loc,
        new_rc.result(ctx),
        zero.result(ctx),
...
fn gen_deep_release_call_arena(
    ...
    // size = iconst(alloc_size)
    let size = arena_clif::iconst(ctx, loc, i64_ty, alloc_size as i64);
    ctx.push_op(block, size.op_ref());

    // call @__tribute_deep_release(payload_ptr, size)
    let call = arena_clif::call(
        ctx,
        loc,
        [payload_ptr, size.result(ctx)],
        nil_ty,
        Symbol::new(ARENA_DEEP_RELEASE_FN),
    );
    ctx.push_op(block, call.op_ref());

    // jump to continue block
    let jump = arena_clif::jump(ctx, loc, [], continue_block);
