...
use trunk_ir::arena::{BlockRef, OpRef, RegionRef, TypeRef, ValueDef as ArenaValueDef, ValueRef};

use tribute_ir::arena::dialect::tribute_rt as arena_tribute_rt;

/// Arena: Check if a type is `core.ptr`.
fn is_ptr_type_arena(ctx: &IrContext, ty: TypeRef) -> bool {
    let data = ctx.types.get(ty);
    data.dialect == Symbol::new("core") && data.name == Symbol::new("ptr")
}

/// Arena: Check if a value is a pointer type.
fn is_ptr_value_arena(ctx: &IrContext, value: ValueRef) -> bool {
    is_ptr_type_arena(ctx, ctx.value_ty(value))
}

/// Arena: Check if an op is a block terminator.
fn is_terminator_op_arena(ctx: &IrContext, op: OpRef) -> bool {
...
/// Arena: Check if a value is a static pointer (not RC-managed).
fn is_static_ptr_arena(ctx: &IrContext, value: ValueRef) -> bool {
    let ArenaValueDef::OpResult(def_op, _) = ctx.value_def(value) else {
        return false;
    };
    if arena_clif::SymbolAddr::matches(ctx, def_op) {
        return true;
    }
    if arena_clif::Iconst::matches(ctx, def_op) && is_ptr_type_arena(ctx, ctx.value_ty(value)) {
        return true;
    }
    false
}

/// Arena: Check if a value is an intermediate allocation pointer.
fn is_alloc_intermediate_arena(ctx: &IrContext, value: ValueRef) -> bool {
    let ArenaValueDef::OpResult(def_op, _) = ctx.value_def(value) else {
        return false;
    };
    ...
    if let Ok(_iadd_op) = arena_clif::Iadd::from_op(ctx, def_op) {
        ...
        if let Some(&lhs) = operands.first() {
            let ArenaValueDef::OpResult(lhs_op, _) = ctx.value_def(lhs) else {
                return false;
            };
...
/// Arena: Infer allocation size by tracing the def chain.
fn infer_alloc_size_arena(ctx: &IrContext, value: ValueRef) -> u64 {
    let ArenaValueDef::OpResult(def_op, _) = ctx.value_def(value) else {
        return 0;
    };
    let Ok(call_op) = arena_clif::Call::from_op(ctx, def_op) else {
        if let Ok(_iadd_op) = arena_clif::Iadd::from_op(ctx, def_op) {
            let operands = ctx.op_operands(def_op).to_vec();
            if let Some(&lhs) = operands.first() {
                return infer_alloc_size_arena(ctx, lhs);
            }
        }
        return 0;
    };
    ...
    let ArenaValueDef::OpResult(size_op, _) = ctx.value_def(size_val) else {
        return 0;
    };
...
/// Arena: Per-block liveness information.
struct LivenessInfoArena {
...
/// Arena: Collect all pointer-typed values in the function body.
fn collect_ptr_values_arena(ctx: &IrContext, body: RegionRef) -> HashSet<ValueRef> {
    let mut ptr_values = HashSet::new();
    ...
    for &block in &blocks {
        ...
        for &arg_val in ctx.block_args(block) {
            if is_ptr_type_arena(ctx, ctx.value_ty(arg_val)) {
        ...
        for &op in &ctx.block(block).ops.to_vec() {
            for &result_val in ctx.op_results(op) {
                if is_ptr_type_arena(ctx, ctx.value_ty(result_val))
                    && !is_static_ptr_arena(ctx, result_val)
                {
    ...
    for &block in &blocks {
        for &op in &ctx.block(block).ops.to_vec() {
            for &operand in ctx.op_operands(op) {
                if is_ptr_value_arena(ctx, operand) && !is_static_ptr_arena(ctx, operand) {
                    ptr_values.insert(operand);
                }
            }
        }
...
/// Arena: Build alias map for unrealized_conversion_cast.
fn build_ptr_alias_map_arena(
    ctx: &IrContext,
    body: RegionRef,
...
/// Arena: Build CFG successor map.
fn build_successor_map_arena(ctx: &IrContext, body: RegionRef) -> HashMap<BlockRef, Vec<BlockRef>> {
...
/// Arena: Compute use and def sets.
fn compute_use_def_sets_arena(
    ctx: &IrContext,
    body: RegionRef,
...
/// Arena: Compute liveness via backward dataflow.
fn compute_liveness_arena(
    ctx: &IrContext,
    body: RegionRef,
    ptr_values: &HashSet<ValueRef>,
    ptr_alias_map: &HashMap<ValueRef, ValueRef>,
) -> LivenessInfoArena {
    let (use_sets, def_sets) = compute_use_def_sets_arena(ctx, body, ptr_values, ptr_alias_map);
    let successor_map = build_successor_map_arena(ctx, body);
...
/// Arena: Insertion plan for RC operations.
#[derive(Default)]
struct InsertionPlanArena {
    before: HashMap<usize, Vec<OpRef>>,
    after: HashMap<usize, Vec<OpRef>>,
    at_start: Vec<OpRef>,
}

/// Arena: Check if value has any use after given op index in block.
fn has_use_after_arena(
    ctx: &IrContext,
    ops: &[OpRef],
    after_idx: usize,
    value: ValueRef,
...
/// Arena: Check if value is defined before given op index.
fn is_defined_before_arena(
    ctx: &IrContext,
    ops: &[OpRef],
    before_idx: usize,
    value: ValueRef,
    ...
    match ctx.value_def(value) {
        ArenaValueDef::BlockArg(_, _) => true,
        ArenaValueDef::OpResult(def_op, _) => {
            for (i, &op) in ops.iter().enumerate() {
                if i >= before_idx {
                    return false;
                }
...
/// Arena: Check if an operation is `clif.call @__tribute_yield`.
fn is_yield_call_arena(ctx: &IrContext, op: OpRef) -> bool {
...
/// Arena: Insert RC operations for all pointer-typed values.
pub fn insert_rc_arena(ctx: &mut IrContext, module: ArenaModule) {
    ...
    for op in module_ops {
        if let Ok(func_op) = arena_clif::Func::from_op(ctx, op) {
            ...
            insert_rc_in_function_arena(ctx, body);
        }
    }
}

/// Arena: Insert RC in a function body.
fn insert_rc_in_function_arena(ctx: &mut IrContext, body: RegionRef) {
    let mut ptr_values = collect_ptr_values_arena(ctx, body);

    if ptr_values.is_empty() {
        return;
    }

    let ptr_alias_map = build_ptr_alias_map_arena(ctx, body, &mut ptr_values);
    let liveness = compute_liveness_arena(ctx, body, &ptr_values, &ptr_alias_map);

    let blocks: Vec<BlockRef> = ctx.region(body).blocks.to_vec();
    for (block_idx, &block) in blocks.iter().enumerate() {
        insert_rc_in_block_arena(
            ctx,
            block,
            block_idx == 0,
            &ptr_values,
...
/// Arena: Insert RC ops in a single block.
fn insert_rc_in_block_arena(
    ctx: &mut IrContext,
    block: BlockRef,
    is_entry: bool,
    ptr_values: &HashSet<ValueRef>,
    liveness: &LivenessInfoArena,
    ptr_alias_map: &HashMap<ValueRef, ValueRef>,
) {
    ...
    let mut plan = InsertionPlanArena::default();
    ...
    for (op_idx, &op) in ops.iter().enumerate() {
        ...
        if arena_clif::Load::matches(ctx, op) {
            ...
            if result_ty.is_some_and(|ty| is_ptr_type_arena(ctx, ty)) {
    ...
    for (op_idx, &op) in ops.iter().enumerate() {
        ...
        let live_across_yield: Vec<ValueRef> = {
            ...
            live.sort_by_key(|v| match ctx.value_def(*v) {
                ArenaValueDef::BlockArg(_, idx) => (0usize, idx),
                ArenaValueDef::OpResult(def_op, idx) => {
                    let pos = ops.iter().position(|&o| o == def_op).unwrap_or(usize::MAX);
        ...
        for (i, v) in live_across_yield.iter().enumerate() {
            ...
            let obj_alloc_size = infer_alloc_size_arena(ctx, *v);
    ...
    for v in &defs_in_block {
        if !live_out.contains(v)
            ...
            && !is_alloc_intermediate_arena(ctx, *v)
        {
            dying_values.insert(*v);
        }
    }
    ...
    for v in &dying_sorted {
        ...
        } else if let ArenaValueDef::OpResult(def_op, _) = ctx.value_def(*v) {
            for (op_idx, &op) in ops.iter().enumerate() {
                if op == def_op {
                    let alloc_size = infer_alloc_size_arena(ctx, *v);
                    let op_loc = ctx.op(op).location;
        ...
        } else if let ArenaValueDef::BlockArg(_, _) = ctx.value_def(*v) {
            let alloc_size = infer_alloc_size_arena(ctx, *v);
            let release_op = arena_tribute_rt::release(ctx, loc, *v, alloc_size);
            plan.at_start.push(release_op.op_ref());
        }
    ...
    apply_insertion_plan_arena(ctx, block, &ops, &plan);
}

/// Arena: Apply insertion plan by removing and re-inserting ops in order.
fn apply_insertion_plan_arena(
    ctx: &mut IrContext,
    block: BlockRef,
    original_ops: &[OpRef],
    plan: &InsertionPlanArena,
) {
    let has_changes =
        !plan.at_start.is_empty() || !plan.before.is_empty() || !plan.after.is_empty();
    if !has_changes {
