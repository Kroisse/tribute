...
fn build_lookup_loop_body(
    ...
    // mid = (low + high) / 2
    let add_op = wasm_dialect::I32Add::operands(low, high).build(location, ctx);
    ...
    let continue_else = {
        ...
        let update_low = {
            ...
            let add_one = wasm_dialect::I32Add::operands(mid, one.result(ctx)).build(location, ctx);
...
fn generate_evidence_extend_function(ctx: &mut IrContext, location: Location) -> OpRef {
    ...
    let add_len_op = wasm_dialect::I32Add::operands(old_len, one.result(ctx)).build(location, ctx);
    ...
    let copy_suffix_then = {
        ...
        let dst_offset_op =
            wasm_dialect::I32Add::operands(insert_idx, one2.result(ctx)).build(location, ctx);
