...
//!
//! Post-CPS IR uses `core.never` as the logical result of CPS control flow.
//! This target-neutral boundary changes only convention-proven CPS callables
//! to the shared `core.nil` empty-result marker. Native and Wasm own later
//! instruction and signature lowering.
...
/// Exact physical layout metadata for a typed closure callable.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct PhysicalClosureAbi {
    pub convention: CallingConvention,
    pub environment_index: usize,
}

/// Attach the exact callable signature for an indirect transfer.
...
/// Record the typed closure used to construct a canonical runtime closure.
...
/// Read typed closure provenance from a canonical runtime closure pack.
...
/// Build a closure type with exact convention and environment placement.
pub fn physical_closure_type(
    ...
    // Preserve the pre-physicalization interning order for target-neutral
    // pipelines whose printed type aliases are part of stable snapshots.
    ctx.types.intern(
...
/// Read exact physical ABI metadata from an outer closure type.
pub fn get_physical_closure_abi(ctx: &IrContext, closure: TypeRef) -> Option<PhysicalClosureAbi> {
    ...
    let convention = data
        ...
        .try_into()
        .ok()?;
    let environment_index = usize::from(data.attrs.get_u8(CLOSURE_ENVIRONMENT_INDEX_ATTR).ok()??);
    Some(PhysicalClosureAbi {
        convention,
        environment_index,
    })
}
...
/// empty-result marker.
///
/// Validation and type-conversion planning complete before any existing IR is
/// mutated, so a rejected module remains textually unchanged.
pub fn lower_cps_signatures_to_physical(
    ...
    let proven_closures = collect_hidden_cps_closures(ctx, &functions, never)?;

    let aliases = ctx.type_aliases().to_vec();
    let mut converter = PhysicalTypeConverter::new(ctx, never, nil, proven_closures);
    ...
    for &op in &ops {
        ...
        let mut nested_block_args = Vec::new();
        for region in converter.ctx.op(op).regions.iter().copied() {
            for block in converter.ctx.region(region).blocks.iter().copied() {
                nested_block_args.extend(
                    converter
                        .ctx
                        .block(block)
                        .args
                        .iter()
                        .enumerate()
                        .map(|(index, arg)| (block, index as u32, arg.ty)),
                );
            }
        }
        for (block, index, ty) in nested_block_args {
            let converted = converter.convert_embedded(ty)?;
            if converted != ty {
                block_args.push((block, index, converted));
            }
        }
    }
...
fn exact_convention(
    ...
    let has_attribute = ctx.op(op).attributes.get(CALLING_CONVENTION_ATTR).is_some();
    let convention = get_calling_convention(ctx, op);
    if has_attribute && convention.is_none() {
        return Err(TargetAbiError::new(
...
fn validate_transfers(
    ...
    for &op in ops {
        ...
        if func::Call::matches(ctx, op) || func::TailCall::matches(ctx, op) {
            ...
            if !operands_match(ctx, ctx.op_operands(op), signature.params(ctx)) {
                return Err(TargetAbiError::new(
...
fn operands_match(ctx: &IrContext, operands: &[trunk_ir::ValueRef], params: &[TypeRef]) -> bool {
    operands.len() == params.len()
        && operands
            .iter()
            .zip(params)
            .all(|(value, ty)| ctx.value_ty(*value) == *ty)
}
...
fn function_for_symbol(
    ...
    function_for_symbol_optional(ctx, op, symbol, functions)?.ok_or_else(|| {
        TargetAbiError::new(format!(
            "target ABI: unknown convention-proven callable `{symbol}`"
        ))
    })
}
...
fn collect_hidden_cps_closures(
    ctx: &IrContext,
    functions: &HashMap<(OpRef, Symbol), FunctionIdentity>,
    never: TypeRef,
) -> Result<HashMap<TypeRef, PhysicalClosureAbi>, TargetAbiError> {
    let mut proven = HashMap::new();
    for identity in functions.values().copied() {
        if identity.convention != CallingConvention::Cps {
            continue;
        }
        let callable = core::Func::from_type_ref(ctx, identity.signature).unwrap();
        let logical_done_index = 1;
        let physical_done_index = logical_done_index
            + usize::from(
                identity
                    .environment_index
                    .is_some_and(|index| index <= logical_done_index),
            );
        let Some(&done_k) = callable.params(ctx).get(physical_done_index) else {
            return Err(TargetAbiError::new(
                "target ABI: Cps callable is missing its done continuation",
            ));
        };
        let data = ctx.types.get(done_k);
        if data.dialect != Symbol::new("closure") || data.name != Symbol::new("closure") {
            continue;
        }
        let [inner] = data.params.as_slice() else {
            return Err(TargetAbiError::new(
                "target ABI: done continuation closure must contain one callable",
            ));
        };
        let inner = core::Func::from_type_ref(ctx, *inner).ok_or_else(|| {
            TargetAbiError::new("target ABI: done continuation must contain core.func")
        })?;
        if inner.r#return(ctx) != never {
            return Err(TargetAbiError::new(
                "target ABI: done continuation must have logical core.never result",
            ));
        }
        let exact = PhysicalClosureAbi {
            convention: CallingConvention::Cps,
            environment_index: 0,
        };
        if let Some(previous) = proven.insert(done_k, exact)
            && previous != exact
        {
            return Err(TargetAbiError::new(
                "target ABI: conflicting done continuation closure metadata",
            ));
        }
    }
    Ok(proven)
...
struct PhysicalTypeConverter<'a> {
    ...
    proven_closures: HashMap<TypeRef, PhysicalClosureAbi>,
}
...
impl<'a> PhysicalTypeConverter<'a> {
    ...
    fn convert_embedded(&mut self, ty: TypeRef) -> Result<TypeRef, TargetAbiError> {
        ...
        if data.dialect == Symbol::new("closure") && data.name == Symbol::new("closure") {
            ...
            let inner_callable = core::Func::from_type_ref(self.ctx, *inner)
                .ok_or_else(|| TargetAbiError::new("target ABI: closure must contain core.func"))?;
            let abi = get_physical_closure_abi(self.ctx, ty)
                .or_else(|| self.proven_closures.get(&ty).copied());
            let Some(abi) = abi else {
                if inner_callable.r#return(self.ctx) == self.never {
                    return Err(TargetAbiError::new(
                        "target ABI: logical core.never closure has no exact physical ABI metadata",
                    ));
                }
                let mut converted = data.clone();
                converted.params[0] = self.convert_embedded(*inner)?;
            ...
            if abi.environment_index
                > core::Func::from_type_ref(self.ctx, *inner)
                    .map(|function| function.params(self.ctx).len())
                    .unwrap_or_default()
            {
                return Err(TargetAbiError::new(
                    "target ABI: closure environment index is outside callable signature",
                ));
            }
            let mut converted = data.clone();
            converted.params[0] = self.convert_callable(*inner, abi.convention)?;
            converted.attrs.insert(
                Symbol::new(CALLING_CONVENTION_ATTR),
                Attribute::Int(abi.convention as i128),
            );
            converted.attrs.insert(
                Symbol::new(CLOSURE_ENVIRONMENT_INDEX_ATTR),
                Attribute::Int(abi.environment_index as i128),
            );
            self.convert_type_attributes(&mut converted)?;
            let converted = self.intern_if_changed(ty, converted);
            self.embedded.insert(ty, converted);
...
mod tests {
    ...
    #[test]
    fn direct_and_evidence_never_results_remain_logical_but_cps_is_physical() {
        let mut ctx = IrContext::new();
        let module = parse_test_module(
            &mut ctx,
            r#"core.module @test {
  !done = closure.closure(core.func(core.never, core.i32)) {tribute.calling_convention = 2, tribute.closure_environment_index = 0}
  ...
  func.func @cps(%ev: core.i32, %done: !done) -> core.never attributes {tribute.calling_convention = 2} { func.unreachable }
}"#,
        );
    ...
    #[test]
    fn physicalizes_exact_indirect_cps_signature() {
        let mut ctx = IrContext::new();
        let module = parse_test_module(
            &mut ctx,
            r#"core.module @test {
  func.func @run(%callee: core.i32, %value: core.i32) -> core.never attributes {tribute.calling_convention = 2} {
    ...
    #[test]
    fn synthesized_continuation_uses_checked_environment_slot() {
  ...
  tribute_control.func @run(%value: core.i32) -> core.i32 convention(cps) {
    %called = tribute_control.call %value {callee = @id} : core.i32
    %sum = arith.addi %called, %value : core.i32
    ...
    #[test]
    fn malformed_direct_tail_is_rejected_without_mutation() {
        let mut ctx = IrContext::new();
        let module = parse_test_module(
            &mut ctx,
            r#"core.module @test {
  ...
  func.func @caller(%value: core.bool) -> core.never attributes {tribute.calling_convention = 2} {
    ...
    #[test]
    fn indirect_tail_without_exact_signature_is_rejected_without_mutation() {
        let mut ctx = IrContext::new();
        let module = parse_test_module(
            &mut ctx,
            r#"core.module @test {
  func.func @caller(%callee: core.func(core.never)) -> core.never attributes {tribute.calling_convention = 2} {
    func.tail_call_indirect %callee {tribute.calling_convention = 2}
        ...
        assert!(error.to_string().contains("lacks exact callable signature"));
        assert_eq!(print_module(&ctx, module.op()), before);
    }

    #[test]
    fn indirect_tail_outside_cps_caller_is_rejected_without_mutation() {
        let mut ctx = IrContext::new();
        let module = parse_test_module(
            &mut ctx,
            r#"core.module @test {
  func.func @caller(%callee: core.func(core.never)) -> core.i32 attributes {tribute.calling_convention = 0} {
    func.tail_call_indirect %callee {tribute.calling_convention = 2, func.indirect_call_signature = core.func(core.never)}
        ...
        assert!(error.to_string().contains("Cps core.never transfer"));
        assert_eq!(print_module(&ctx, module.op()), before);
    }
}
