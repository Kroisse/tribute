...
fn transform_handler_root_function<'db>(
    ...
    let (new_entry_block, block_changed) = transform_shifts_in_block_with_remap(
        db,
        entry_block,
        ev_value,
        &handled_by_tag,
...
///
/// Assumes call_indirect operands are laid out as `[table_idx, evidence, ...rest]`.
/// This is the calling convention for lifted lambdas with evidence passing.
/// Returns the new operation if the call was rewritten, or `None` if not applicable.
fn try_replace_call_indirect_evidence<'db>(
    ...
    // Invariant: operand layout is [table_idx, evidence, ...rest].
    // If this changes, this index must be updated.
...
fn i32_type_ref_arena(ctx: &mut IrContext) -> TypeRef {
    ctx.types
        .intern(TypeDataBuilder::new(Symbol::new("core"), Symbol::new("i32")).build())
}

/// Create a `core.nil` type ref in arena.
fn nil_type_ref_arena(ctx: &mut IrContext) -> TypeRef {
    ctx.types
        .intern(TypeDataBuilder::new(Symbol::new("core"), Symbol::new("nil")).build())
}

/// Create the `cont.prompt_tag` type ref.
fn prompt_tag_type_ref(ctx: &mut IrContext) -> TypeRef {
    ctx.types
        .intern(TypeDataBuilder::new(Symbol::new("cont"), Symbol::new("prompt_tag")).build())
}

/// Ensure runtime helper functions exist in the arena module.
fn ensure_runtime_functions_arena(ctx: &mut IrContext, module: ArenaModule) {
    ...
    if !has_lookup {
        ...
        let i32_ty = i32_type_ref_arena(ctx);
...
/// Collect functions with evidence first parameter (arena).
fn collect_functions_with_evidence_arena(ctx: &IrContext, module: ArenaModule) -> HashSet<Symbol> {
    let mut fns_with_evidence = HashSet::new();
    for op in module.ops(ctx) {
        if let Ok(func_op) = arena_func::Func::from_op(ctx, op) {
            let func_ty = func_op.r#type(ctx);
            if has_evidence_first_param_arena(ctx, func_ty) {
...
/// Check if a `core.func` type has evidence as its first parameter (arena).
fn has_evidence_first_param_arena(ctx: &IrContext, func_ty: TypeRef) -> bool {
...
fn collect_handler_root_functions_arena(
    ctx: &IrContext,
    module: ArenaModule,
    fns_with_evidence: &HashSet<Symbol>,
) -> HashSet<Symbol> {
    ...
    for op in module.ops(ctx) {
        if let Ok(func_op) = arena_func::Func::from_op(ctx, op) {
            ...
            if region_contains_push_prompt_arena(ctx, func_op.body(ctx)) {
                handler_roots.insert(func_name);
            }
        }
    }
...
/// Check if a region contains `cont.push_prompt` (arena).
fn region_contains_push_prompt_arena(ctx: &IrContext, region: RegionRef) -> bool {
    for &block in ctx.region(region).blocks.iter() {
        for &op in ctx.block(block).ops.iter() {
            ...
            for &nested in ctx.op(op).regions.iter() {
                if region_contains_push_prompt_arena(ctx, nested) {
                    return true;
                }
            }
        }
...
/// Collect handled abilities by tag from handler_dispatch ops in a block (arena).
fn collect_handled_abilities_by_tag_arena(
    ctx: &IrContext,
    block: BlockRef,
...
/// Collect operations for a tag from handler_dispatch (arena).
fn collect_operations_for_tag_arena(
    ctx: &IrContext,
    block: BlockRef,
    target_tag: u32,
) -> Vec<(TypeRef, Symbol)> {
...
/// Compute ability ID from an arena TypeRef.
fn compute_ability_id_arena(ctx: &IrContext, ability_ref: TypeRef) -> u32 {
    ...
    let ability_name = match data.attrs.get(&Symbol::new("name")) {
        ...
        _ => panic!(
            "ICE: compute_ability_id_arena: ability type has no name: {:?}",
            data
        ),
    };
    ...
    for &param in data.params.iter() {
        hash = hash.wrapping_mul(37);
        hash = hash.wrapping_add(hash_type_arena(ctx, param));
    }

    hash
}

/// Hash a type for ability ID computation (arena).
fn hash_type_arena(ctx: &IrContext, ty: TypeRef) -> u32 {
...
/// Validate ability ID uniqueness (arena).
fn validate_ability_id_uniqueness_arena(ctx: &IrContext, module: ArenaModule) {
    let mut seen: HashMap<u32, TypeRef> = HashMap::new();
    for op in module.ops(ctx) {
        if let Ok(func_op) = arena_func::Func::from_op(ctx, op) {
            collect_ability_ids_in_region_arena(ctx, func_op.body(ctx), &mut seen);
        }
    }
}

fn collect_ability_ids_in_region_arena(
    ctx: &IrContext,
    region: RegionRef,
    ...
    for &block in ctx.region(region).blocks.iter() {
        for &op in ctx.block(block).ops.iter() {
            if let Ok(shift_op) = arena_cont::Shift::from_op(ctx, op) {
                ...
                let id = compute_ability_id_arena(ctx, ability_ref);
                if let Some(existing) = seen.get(&id) {
                    if *existing != ability_ref {
                        panic!("ICE: ability ID hash collision in arena: both hash to {id:#010x}");
                    }
                } else {
                    seen.insert(id, ability_ref);
                }
            ...
            for &nested in ctx.op(op).regions.iter() {
                collect_ability_ids_in_region_arena(ctx, nested, seen);
            }
        }
    }
}

/// Validate no unresolved shifts remain (arena).
fn validate_no_unresolved_shifts_arena(ctx: &IrContext, module: ArenaModule) {
    for op in module.ops(ctx) {
        if let Ok(func_op) = arena_func::Func::from_op(ctx, op) {
            validate_no_unresolved_shifts_in_region_arena(ctx, func_op.body(ctx));
        }
    }
}
...
fn transform_handler_roots_arena(
    ctx: &mut IrContext,
    module: ArenaModule,
    handler_root_fns: &HashSet<Symbol>,
    fns_with_evidence: &HashSet<Symbol>,
    registry: &mut ArenaOpTableRegistry,
) {
    ...
    for func_op_ref in func_ops {
        ...
        let handled_by_tag = collect_handled_abilities_by_tag_arena(ctx, entry_block);
        transform_shifts_in_block_arena(
            ctx,
            entry_block,
            ev_value,
            &handled_by_tag,
        ...
        for &block in blocks.iter().skip(1) {
            let handled_by_tag = collect_handled_abilities_by_tag_arena(ctx, block);
            transform_shifts_in_block_arena(
                ctx,
                block,
                ev_value,
...
/// Transform shifts in all functions that have evidence (arena).
fn transform_shifts_in_module_arena(
    ctx: &mut IrContext,
    module: ArenaModule,
    fns_with_evidence: &HashSet<Symbol>,
    registry: &mut ArenaOpTableRegistry,
...
/// Transform shifts in a single block (arena).
///
...
#[allow(clippy::too_many_arguments)]
fn transform_shifts_in_block_arena(
    ctx: &mut IrContext,
    block: BlockRef,
    ev_value: ValueRef,
    ...
    for op in ops {
        ...
        if let Ok(push_prompt_op) = arena_cont::PushPrompt::from_op(ctx, op) {
            ...
            if abilities.is_empty() {
                ...
                transform_shifts_in_region_arena(
                    ctx,
                    body_region,
                    ev_value,
                    registry,
            ...
            let operations = collect_operations_for_tag_arena(ctx, dispatch_blk, tag);
            ...
            let mut current_ev = ev_value;
            for &ability_ref in &abilities {
                let ability_id = compute_ability_id_arena(ctx, ability_ref);
        ...
        if let Ok(shift_op) = arena_cont::Shift::from_op(ctx, op) {
            ...
            let result_ty = ctx
                ...
                .first()
                .copied()
                .unwrap_or_else(|| nil_type_ref_arena(ctx));
...
/// Transform shifts in a region (arena).
fn transform_shifts_in_region_arena(
    ctx: &mut IrContext,
    region: RegionRef,
    ev_value: ValueRef,
    registry: &mut ArenaOpTableRegistry,
...
/// Resolve evidence-based dispatch using arena IR.
///
/// This is the arena equivalent of `resolve_evidence_dispatch`.
pub fn resolve_evidence_dispatch_arena(ctx: &mut IrContext, module: ArenaModule) {
    let mut registry = ArenaOpTableRegistry::new();

    // Ensure runtime helpers exist
    ensure_runtime_functions_arena(ctx, module);

    // Validate ability ID uniqueness
    validate_ability_id_uniqueness_arena(ctx, module);

    // Collect functions with evidence
    let fns_with_evidence = collect_functions_with_evidence_arena(ctx, module);

    // Transform handler-root functions first
    let handler_root_fns = collect_handler_root_functions_arena(ctx, module, &fns_with_evidence);
    if !handler_root_fns.is_empty() {
        transform_handler_roots_arena(
            ctx,
            module,
            &handler_root_fns,
            &fns_with_evidence,
    ...
    if !fns_with_evidence.is_empty() {
        transform_shifts_in_module_arena(ctx, module, &fns_with_evidence, &mut registry);
    }

    // Validate no unresolved shifts remain
    validate_no_unresolved_shifts_arena(ctx, module);
}
...
mod tests {
    ...
    // ========================================================================
    // transform_shifts_in_block_with_remap / transform_shifts_in_region_with_remap tests
    // ========================================================================

    /// Test that `transform_shifts_in_block_with_remap` correctly applies non-empty
    ...
    fn run_initial_remap_test(db: &dyn salsa::Database) -> Result<(), String> {
        ...
        let registry = Rc::new(RefCell::new(OpTableRegistry::new()));
        let handled_by_tag = HashMap::new();

        let (new_block, changed) = transform_shifts_in_block_with_remap(
            db,
            &block,
            ev_value,
            &handled_by_tag,
    ...
    #[salsa_test]
    fn test_transform_shifts_in_block_with_remap_uses_initial_remap(db: &salsa::DatabaseImpl) {
    ...
    /// Test that `transform_shifts_in_region_with_remap` propagates `initial_remap`
    ...
    fn run_region_remap_propagation_test(db: &dyn salsa::Database) -> Result<(), String> {
        ...
        let (new_region, changed) = transform_shifts_in_region_with_remap(
            db,
            &region,
            ev_value,
            initial_remap,
    ...
    #[salsa_test]
    fn test_transform_shifts_in_region_with_remap_propagates_remap(db: &salsa::DatabaseImpl) {
