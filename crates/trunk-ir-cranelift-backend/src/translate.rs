...
/// Trap used when a dynamically released value has no RTTI release entry.
const UNRESOLVED_DYNAMIC_RELEASE_TRAP_CODE: u8 = 2;
...
fn build_shallow_only_deep_release(
    ...
    builder.ins().trap(cl_ir::TrapCode::unwrap_user(
        UNRESOLVED_DYNAMIC_RELEASE_TRAP_CODE,
    ));
}
