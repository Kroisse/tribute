...
use trunk_ir::{Attribute, DialectOp, DialectType, Operation, Type};
...
/// Lower arith dialect to clif dialect (arena IR).
pub fn lower_arena(ctx: &mut IrContext, module: ArenaModule, type_converter: ArenaTypeConverter) {
    ...
    let applicator = ArenaPatternApplicator::new(type_converter)
        .with_target(target)
        .add_pattern(ArenaArithConstPattern)
        .add_pattern(ArenaArithBinOpPattern)
        .add_pattern(ArenaArithCmpPattern)
        .add_pattern(ArenaArithNegPattern)
        .add_pattern(ArenaArithBitwisePattern)
        .add_pattern(ArenaArithConversionPattern);
    applicator.apply_partial(ctx, module);
}

/// Classify arena type into integer vs float category (for clif lowering).
fn arena_type_category(ctx: &IrContext, ty: Option<TypeRef>) -> &'static str {
    match ty {
        Some(t) => {
...
fn arena_is_unsigned_int(ctx: &IrContext, ty: Option<TypeRef>) -> bool {
    match ty {
        Some(t) => {
            let name = ctx.types.get(t).name;
...
fn arena_is_wider_int(ctx: &IrContext, dst: TypeRef, src: Option<TypeRef>) -> bool {
...
struct ArenaArithConstPattern;

impl ArenaRewritePattern for ArenaArithConstPattern {
    fn match_and_rewrite(
        ...
        let category = arena_type_category(ctx, Some(result_ty));
...
struct ArenaArithBinOpPattern;

impl ArenaRewritePattern for ArenaArithBinOpPattern {
    fn match_and_rewrite(
        &self,
        ctx: &mut IrContext,
        op: OpRef,
        ...
        let category = arena_type_category(ctx, Some(result_ty));
        let loc = ctx.op(op).location;
        let is_unsigned = arena_is_unsigned_int(ctx, Some(result_ty));
...
struct ArenaArithCmpPattern;

impl ArenaRewritePattern for ArenaArithCmpPattern {
    fn match_and_rewrite(
        &self,
        ctx: &mut IrContext,
        op: OpRef,
        ...
        let category = arena_type_category(ctx, operand_ty);
        let is_float = matches!(category, "f32" | "f64");
        let is_unsigned = !is_float && arena_is_unsigned_int(ctx, operand_ty);
...
struct ArenaArithNegPattern;

impl ArenaRewritePattern for ArenaArithNegPattern {
    fn match_and_rewrite(
        &self,
        ctx: &mut IrContext,
        op: OpRef,
...
struct ArenaArithBitwisePattern;

impl ArenaRewritePattern for ArenaArithBitwisePattern {
    fn match_and_rewrite(
        &self,
        ctx: &mut IrContext,
        op: OpRef,
...
struct ArenaArithConversionPattern;

impl ArenaRewritePattern for ArenaArithConversionPattern {
    fn match_and_rewrite(
        &self,
        ctx: &mut IrContext,
        op: OpRef,
        ...
        let src_cat = arena_type_category(ctx, Some(src_ty));

        let Some(dst_ty) = rewriter.result_type(ctx, op, 0) else {
            return false;
        };
        let dst_cat = arena_type_category(ctx, Some(dst_ty));
        ...
        let new_op = if name == Symbol::new("cast") {
            match (src_cat, dst_cat) {
                ("int", "int") => {
                    if arena_is_wider_int(ctx, dst_ty, Some(src_ty)) {
            ...
            match (src_cat, dst_cat) {
                ("int", "int") if arena_is_unsigned_int(ctx, Some(src_ty)) => {
