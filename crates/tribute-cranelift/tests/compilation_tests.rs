...
const TEST_CASES: &[(&str, &str, bool)] = &[
    ...
    ("basic.trb", include_str!("../../../lang-examples/basic.trb"), true),
    ("functions.trb", include_str!("../../../lang-examples/functions.trb"), true),
    ("pattern_matching.trb", include_str!("../../../lang-examples/pattern_matching.trb"), true),
    ("pattern_advanced.trb", include_str!("../../../lang-examples/pattern_advanced.trb"), true),
    ("hello.trb", include_str!("../../../lang-examples/hello.trb"), true),
    ("let_simple.trb", include_str!("../../../lang-examples/let_simple.trb"), true),
    ("let_bindings.trb", include_str!("../../../lang-examples/let_bindings.trb"), true),
    ("calc.trb", include_str!("../../../lang-examples/calc.trb"), true),
    ...
    for &(name, source, should_compile) in TEST_CASES {
        if should_compile {
            ...
            if let Ok(object_bytes) = result {
                ...
                assert!(!object_bytes.is_empty(), "Object file for {} is empty", name);
                assert!(object_bytes.len() > 100, "Object file for {} is suspiciously small: {} bytes", name, object_bytes.len());
                ...
                assert!(is_valid_object, "Object file for {} doesn't have valid magic number: {:02x?}", name, magic);
            }
        }
    }
}
...
fn test_compilation_failure_cases() {
    for &(name, source, should_compile) in TEST_CASES {
        if !should_compile {
            let result = compile_source(name, source);
            assert!(result.is_err(), "Expected {} to fail compilation, but it succeeded", name);
        }
    }
}
...
fn test_real_example_files() {
    ...
    for example_name in successful_examples {
        ...
        if example_path.exists() {
            ...
            assert!(result.is_ok(), "Failed to compile example {}: {:?}", example_name, result.err());
        }
    }
}
...
fn test_syntax_error() {
    ...
    assert!(any_failed, "At least one invalid syntax should cause compilation failure");
}
...
"#;
    ...
    assert_eq!(result1.len(), result2.len(), "Object file sizes should be identical");
