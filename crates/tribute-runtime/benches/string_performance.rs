...
fn bench_string_creation(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        ("very_long_50", "this is a much longer string that definitely goes to heap allocation table"), // Heap mode
    ];
    
    for (name, text) in test_cases {
        group.bench_with_input(BenchmarkId::new("inline_or_heap", name), &text, |b, text| {
            b.iter(|| {
                let handle = tr_value_from_string(black_box(text.as_ptr()), black_box(text.len()));
        ...
        group.bench_with_input(BenchmarkId::new("static", name), &text, |b, text| {
            b.iter(|| {
                ...
                let handle = tr_value_from_static_string(black_box(0), black_box(text.len() as u32));
                tr_value_free(black_box(handle));
            });
        });
    }
