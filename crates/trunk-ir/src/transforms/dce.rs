...
mod tests {
    ...
    fn cascade_removal() {
        ...
        let func_op = build_func(&mut ctx, loc, "main", |ctx, loc, entry| {
            ...
            let b = arith::Addi::operands(a.result(ctx), a.result(ctx)).build(loc, ctx);
            ctx.push_op(entry, b.op_ref());

            let c = arith::Addi::operands(b.result(ctx), a.result(ctx)).build(loc, ctx);
    ...
    fn keeps_used_ops() {
        ...
        let func_op = build_func(&mut ctx, loc, "main", |ctx, loc, entry| {
            ...
            ctx.push_op(entry, b.op_ref());
            let c = arith::Addi::operands(a.result(ctx), b.result(ctx)).build(loc, ctx);
            ctx.push_op(entry, c.op_ref());
