...
fn test_arithmetic_operations() {
    ...
    let result = eval_expr(&mut env, &add_expr.0).unwrap();
    ...
    let result = eval_expr(&mut env, &sub_expr.0).unwrap();
    ...
    let result = eval_expr(&mut env, &mul_expr.0).unwrap();
    ...
    let result = eval_expr(&mut env, &div_expr.0).unwrap();
...
fn test_string_manipulation() {
    ...
    let result = eval_expr(&mut env, &trim_expr.0).unwrap();
    ...
    let result = eval_expr(&mut env, &invalid_num_expr.0);
    assert!(result.is_err());
}
...
fn test_collection_functions() {
    ...
    let result = eval_expr(&mut env, &get_oob_expr.0);
    assert!(result.is_err());
}
...
fn test_match_case() {
    ...
    let result = eval_expr(&mut env, &match_num_expr.0).unwrap();
    ...
    let result = eval_expr(&mut env, &no_match_expr.0);
    assert!(result.is_err());
}
...
fn test_function_definition_and_call() {
    ...
    let result = eval_expr(&mut env, &fn_def.0).unwrap();
    ...
    let result = eval_expr(&mut env, &fn_call.0).unwrap();
    assert!(matches!(result, Value::Number(6)));
}
...
fn test_let_binding() {
    ...
    let result = eval_expr(&mut env, &let_expr.0).unwrap();
    assert!(matches!(result, Value::Unit));
    
    // Test that the binding works
    let var_expr = expr_ident("x");
    let result = eval_expr(&mut env, &var_expr.0).unwrap();
    assert!(matches!(result, Value::Number(42)));
}